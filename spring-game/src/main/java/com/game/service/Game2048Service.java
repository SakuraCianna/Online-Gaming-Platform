...
public class Game2048Service {
    ...
    public Game2048Service(Game2048Mapper game2048Mapper, GameRecordService gameRecordService) {
    ...
    public Map<String, Object> saveData(Map<String, Object> request) {
        ...
        insert.put("max_tile", maxTile);
        insert.put("moves_count", movesCount);
        ...
        try {
            if (game2048Mapper.insert2048Record(insert) == 1) {
                result.put("success", true);
                result.put("message", "游戏保存成功");

                // 失效缓存
                gameRecordService.invalidateCache(userId, "2048");
            }
        } catch (Exception e) {
    ...
    public Map<String, Object> updateData(Map<String, Object> request) {
        ...
        update.put("max_tile", maxTile);
        update.put("moves_count", movesCount);
        ...
        try {
            if (game2048Mapper.update2048Record(update) == 1) {
                result.put("success", true);
