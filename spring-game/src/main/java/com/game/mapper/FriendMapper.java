...
public interface FriendMapper extends BaseMapper<Friend> {
    ...
    @Select("SELECT EXISTS(Select 1 from friend where user_id = #{user_id} and friend_id = #{friend_id} and status = 1)")
    boolean isFriend(@Param("user_id") long id, @Param("friend_id") long friend_id);
    ...
    boolean existFriendData(@Param("userId") long userId, @Param("friendId") long friendId, @Param("status") int status);
